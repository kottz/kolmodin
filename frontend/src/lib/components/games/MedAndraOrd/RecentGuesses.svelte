<!-- src/lib/components/games/MedAndraOrd/RecentGuesses.svelte -->
<script lang="ts">
	...
	<CardHeader>
		...
		<CardDescription>
			{#if recentGuesses.length === 0}
				No recent guesses yet
			{:else}
				Last {recentGuesses.length} correct guesses - click X to remove and deduct point
			{/if}
		</CardDescription>
	</CardHeader>
	<CardContent>
		{#if recentGuesses.length === 0}
			<div class="text-muted-foreground py-8 text-center">
				...
				<p>Recent correct guesses will appear here</p>
			</div>
		{:else}
			<div class="space-y-3">
				{#each recentGuesses as guess (guess.id)}
					<div class="bg-muted/50 flex items-center justify-between rounded-lg border p-3">
						<div class="flex-1 space-y-1">
							...
							<div class="text-sm">
								Word: <span class="bg-muted rounded px-1 py-0.5 font-mono text-xs font-medium"
						...
						<Button
							...
							title="Remove guess and deduct 1 point from {guess.player}"
						>
							✕
						</Button>
					</div>
